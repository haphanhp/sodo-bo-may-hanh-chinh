  ...
  help: {
    ...
    render: () => head("Hướng dẫn sử dụng", "Cách dùng ứng dụng và nguyên tắc dữ liệu của dự án.") + `
      <div class="card" style="padding:24px"><div class="prose">
        ...
        <p>Đang ở <strong>Phase 5</strong>: đã có data engine, sơ đồ quan hệ, bảng chi tiết và tìm kiếm toàn cục. Dữ liệu thật gồm 5 cơ quan trung ương, 14 Bộ, 3 cơ quan ngang Bộ và 34 tỉnh/thành phố trực thuộc Trung ương. Thủ tục (Phase 6), văn bản (Phase 7), dữ liệu lịch sử (Phase 9) chưa bật.</p><p class="muted-note">Ghi chú kỹ thuật: trình duyệt chặn đọc file JSON khi mở bằng <code>file://</code> — chạy <code>mo-app.bat</code> trong thư mục dự án (hoặc <code>python -m http.server 8080</code>) rồi mở <code>http://localhost:8080</code>.</p><p style="display:none">Phase 1: mới có khung giao diện, điều hướng và các trạng thái trống. Chưa nạp dữ liệu hành chính thật; dữ liệu thô đã tra cứu nằm trong các file <code>01–16-*.md</code> của dự án và chỉ được ráp vào <code>data/*.json</code> ở Phase 10.</p>
        <h2>4. Phạm vi</h2>
