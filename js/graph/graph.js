...
export function graphMarkup(){
  return `<div class="graph-canvas" id="graph-canvas">
    ...
    <div class="graph-legend">
      ...
      <span><i class="lg n-agency"></i>Cơ quan ngang Bộ / giúp việc</span>
    </div>
...
export function mountGraph(){
  ...
  function draw(){
    ...
    const { nodes, edges } = layout(buildTree(index, { expanded }));
    document.getElementById("graph-stage").innerHTML =
      edges.map(edgeSVG).join("") + nodes.map(n => nodeSVG(n, selectedEntity)).join("");
  ...
  function fit(){
    ...
    const { bounds } = layout(buildTree(index, { expanded }));
