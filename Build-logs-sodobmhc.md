...
#### 2026-09-19 — Bổ sung lộ trình hoàn thiện + ước lượng nguồn lực vào Danh-gia-tong-ket; tạo file Co-che-chong-loi-thoi-sodobmhc.md

- **Việc làm**: Phiên này bắt đầu bằng việc phát hiện dự án đã tiến xa hơn RẤT NHIỀU so với ngữ cảnh phiên đang nhớ (Phase 0→9 đã xong, app thật đã publish, có thêm nhiều file 17–41 và 2 file mới `Danh-gia-tong-ket-sodobmhc.md`/`Script-youtube-sodobmhc.md` do (các) phiên khác tạo) — đã dừng lại đọc đủ `Claude-sodobmhc.md`, `Roadmap-sodobmhc.md`, toàn bộ `Danh-gia-tong-ket-sodobmhc.md` (kể cả phần đang sửa dở, chưa commit) trước khi viết thêm bất cứ gì, đúng luật đọc mục 0. Sau đó: (1) thêm mục "5. Lộ trình hoàn thiện: đầu việc cụ thể + ước lượng nguồn lực" vào `Danh-gia-tong-ket-sodobmhc.md` (đánh số lại 2 mục sau thành 6, 7), liệt kê 9 đầu việc ứng với các khoảng trống đã nêu ở mục 4, mỗi việc kèm ước lượng số prompt mới / số phiên / mức token; (2) tạo file mới `Co-che-chong-loi-thoi-sodobmhc.md` — phân tầng tốc độ lỗi thời (5 tầng), nhiệm kỳ/mốc chính trị cần theo dõi, ngưỡng cảnh báo theo tầng, lịch kiểm tra định kỳ dùng scheduled task, bảng "mốc đã biết trước" (early warning calendar), quy trình xử lý khi phát hiện thay đổi thật, và cách người dùng cuối truy cập nhanh + biết độ mới thông tin; (3) cập nhật `Claude-sodobmhc.md` mục 10 để đăng ký 3 file (`Danh-gia-tong-ket`, `Co-che-chong-loi-thoi`, `Script-youtube`) chưa từng được liệt kê ở đó, và sửa khoảng số file đánh số đã lỗi thời (`01→16` thành `01→41`).
- **Vấn đề gặp**: Không phải lỗi kỹ thuật — mà là rủi ro TỔ CHỨC: nếu phiên này viết thêm nội dung mà KHÔNG đọc trước phần đang sửa dở (uncommitted) của `Danh-gia-tong-ket-sodobmhc.md`, rất có thể đã ghi đè hoặc đánh số mục trùng với phần người khác vừa viết cách đó 10 phút — đúng loại rủi ro mà luật 22 vừa ghi nhận cho file đánh số, nhưng ở đây áp dụng cho cả file KHÔNG đánh số (mọi file dùng chung đều có rủi ro này, không riêng gì file `NN-*.md`).
- **Cách xử lý**: `git diff <tên file>` để xem chính xác phần chưa commit trước khi sửa thêm, không chỉ đọc bản đã commit — vì bản đã commit có thể cũ hơn bản đang nằm trên đĩa. Đã xác nhận không có phiên nào khác đang ghi cùng lúc bằng cách so `mtime` các file vừa sửa với giờ hệ thống hiện tại (cách nhau ~10 phút, không phải đang ghi liên tục) trước khi bắt đầu sửa.
- **Bài học rút ra**:
  - "Đọc file trước khi sửa" (luật đã có từ đầu dự án) cần hiểu là đọc ĐÚNG BẢN TRÊN ĐĨA tại thời điểm sửa (`git diff`/`cat` trực tiếp), không phải bản trong trí nhớ ngữ cảnh của phiên — kể cả khi phiên đó tin rằng mình vừa mới đọc file đó gần đây, vì 1 phiên khác có thể đã sửa nó trong lúc phiên này đang "suy nghĩ".
  - Khi git status cho thấy nhiều file đang ở trạng thái "modified" (uncommitted) lúc bắt đầu phiên, đây là TÍN HIỆU RÕ RÀNG có (hoặc vừa có) phiên khác hoạt động song song trên cùng thư mục — nên coi đây là bước kiểm tra bắt buộc đầu mỗi phiên (`git status --short` + `stat` các file "M"), không phải bước tuỳ chọn.
  - Ước lượng nguồn lực (số prompt/số phiên/token) nên neo vào NHỊP ĐỘ THỰC TẾ đã quan sát được của chính dự án (ví dụ 17 prompt tốn 2–3 phiên cho thủ tục Mức 3) thay vì đoán từ đầu — làm vậy ước lượng đáng tin hơn và người đọc (user) có thể tự kiểm chứng bằng cách đối chiếu với Roadmap.

---

#### 2026-09-19 — Đụng số file lần 3: `19-thu-tuc-hanh-chinh-va-xac-minh-bi-thu.md` trùng số với `19-ten-day-du-nhom5-5tinh.md`, đổi thành `41-...`; thêm luật số 22

- **Việc làm**: Trong lúc có 3 phiên Cowork chạy song song trên cùng thư mục dự án, phát hiện file `19-thu-tuc-hanh-chinh-va-xac-minh-bi-thu.md` (kết quả Prompt #25, Pro tự tra thủ tục hành chính + xác minh Bí thư) trùng số thứ tự với `19-ten-day-du-nhom5-5tinh.md` (do một phiên Claude Code khác tạo trước đó, khi đang tra danh sách tên đầy đủ cấp xã nhóm 5). Đã đổi file bị phát hiện sau thành `41-thu-tuc-hanh-chinh-va-xac-minh-bi-thu.md` — lấy số lớn hơn số lớn nhất đang tồn tại tại thời điểm đổi (không lấp vào các số đang trống ở giữa như 26–29, 36, vốn đã bỏ trống từ 2 lần đụng số trước) — và ghi lý do đổi tên vào trường `ghi-chu-doi-ten` trong frontmatter. Rà soát lại toàn bộ danh sách file đánh số `01-...` đến `41-...`: không còn số nào trùng. Đã thêm luật số 22 vào `Claude-sodobmhc.md` mục 9 (tiêu đề mục đổi từ "21 luật nền" thành "22 luật nền") để chính thức hoá bài học này.
- **Vấn đề gặp**: Đây là lần đụng số file thứ BA được ghi nhận trong dự án (2 lần trước xem các mục log phía trên, ngày 2026-09-18) — xác nhận đây là rủi ro lặp lại có hệ thống chứ không phải sự cố đơn lẻ, mỗi khi có từ 2 phiên AI trở lên chạy song song trên cùng thư mục mà không có cơ chế khoá file.
- **Cách xử lý**: Đổi số file phát hiện SAU (không đổi file đã tồn tại từ trước nếu không chắc ai tạo trước), lấy số lớn hơn số lớn nhất hiện có tại đúng thời điểm phát hiện (không lấp số trống ở giữa), ghi rõ lý do vào frontmatter (`ghi-chu-doi-ten`) để phiên sau đọc hiểu ngay vì sao có việc đổi số, sau đó rà soát lại toàn bộ danh sách số thứ tự để xác nhận không còn trùng.
- **Bài học rút ra**:
  - Số thứ tự file là TÀI NGUYÊN DÙNG CHUNG khi có nhiều phiên AI chạy song song, không phải tài nguyên riêng của phiên đang tạo file — "lấy số tiếp theo" là một hành động GHI vào tài nguyên chung, phải `ls` lại NGAY TẠI THỜI ĐIỂM đặt tên, không dùng số nhớ từ đầu phiên.
  - Luôn lấy số LỚN HƠN số lớn nhất đang tồn tại, TUYỆT ĐỐI KHÔNG lấp vào số đang trống ở giữa — vì số trống đó (do lần đổi số trước để lại) có thể đang được một phiên khác dùng lại đúng lúc mình cũng định dùng.
  - Đã chính thức hoá thành luật số 22 trong `Claude-sodobmhc.md` — xem chi tiết đầy đủ ở đó, không lặp lại nguyên văn ở đây.
