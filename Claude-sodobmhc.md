...
**⚠️ PHẠM VI ĐÃ THU HẸP (user chốt 2026-09-18 — áp dụng từ đây trở đi):**
...
#### 9. 22 luật nền áp dụng cho MỌI AI trong dự án
...
20. **Luôn kiểm tra lại CƠ QUAN CÓ THẨM QUYỀN trước khi tin một bài hướng dẫn thủ tục.** Giai đoạn 2025–2026 có hàng loạt thay đổi mà phần lớn tài liệu trên mạng chưa cập nhật: (a) 01/3/2025 — sáp nhập/giải thể nhiều Bộ (KH&ĐT → Tài chính; GTVT → Xây dựng; TT&TT → KH&CN + VHTTDL; TN&MT + NN&PTNT → NN&MT; LĐ-TB&XH giải thể; lập Bộ Dân tộc và Tôn giáo); (b) 01/3/2025 — giấy phép lái xe chuyển sang Bộ Công an (nộp tại Công an cấp xã / Phòng CSGT); (c) 01/7/2025 — kết thúc cấp huyện, chính quyền 2 cấp, 63 tỉnh còn 34; (d) 30/4/2026 — Đồng Nai lên thành phố trực thuộc TW (27 tỉnh + 7 TP); (e) 01/7/2026 — cấp Phiếu lý lịch tư pháp chuyển từ Sở Tư pháp sang cơ quan Công an (Luật 107/2025/QH15). Thấy tài liệu ghi "UBND cấp huyện", "Sở Giao thông vận tải", "Sở Tư pháp cấp phiếu LLTP" hoặc tên Bộ đã giải thể → coi như dữ liệu lỗi thời, phải tra lại. (Rút ra từ Prompt #25 — 2026-09-18, xem `41-thu-tuc-hanh-chinh-va-xac-minh-bi-thu.md`.)
21. **Không bao giờ nhúng token (PAT) vào URL remote của git.** Đây là quy tắc về CƠ CHẾ, không phải về ý định: khi chạy `git clone https://<token>@github.com/...` hoặc `git remote set-url origin https://<token>@...`, chính git sẽ ghi nguyên văn token vào `.git/config` và giữ ở đó vĩnh viễn — token biến thành plaintext nằm trong thư mục làm việc, lộ với mọi phiên AI, công cụ sync, bản backup và mọi lần chạy `git remote -v` sau này. Repo private KHÔNG bảo vệ được token vì token nằm trên ổ đĩa, không nằm trong repo. Cách đúng: nạp token bằng `git credential approve` (đưa qua stdin, không đặt trên dòng lệnh để khỏi lọt vào lịch sử shell) hoặc `gh auth login --with-token`, rồi dùng remote sạch dạng `https://github.com/<owner>/<repo>.git`. Không echo token, không ghi vào bất kỳ file nào trong repo, không đưa vào message commit; khi xác minh chỉ báo "remote sạch / còn token" thay vì in URL đầy đủ. (Rút ra từ sự cố 2026-09-19: một phiên làm việc trước đã nhúng PAT vào remote của repo `haphanhp/publish`, token nằm plaintext trong `.git/config` cho tới khi được phát hiện.)
22. **Số thứ tự file dữ liệu (`01-...md`, `02-...md`...) là tài nguyên DÙNG CHUNG, không phải tài nguyên riêng của 1 phiên.** Khi nhiều phiên AI (Cowork/Claude Code) chạy song song trên cùng thư mục, việc "lấy số thứ tự tiếp theo" là một hành động GHI vào tài nguyên chung, không phải chỉ đọc. Quy tắc bắt buộc: (a) trước khi đặt tên file mới, PHẢI `ls` lại thư mục NGAY TẠI THỜI ĐIỂM đó (không dùng số nhớ từ đầu phiên); (b) luôn lấy số LỚN HƠN số lớn nhất đang tồn tại, TUYỆT ĐỐI KHÔNG lấp vào một số đang trống ở giữa (ví dụ số bị bỏ trống do một lần đổi số trước đó) — vì chỗ trống đó có thể đang được một phiên khác dùng lại cùng lúc; (c) nếu phát hiện trùng số sau khi đã ghi xong, đổi số của file mình vừa tạo (không đổi số file đã có từ trước nếu không chắc ai tạo trước), lấy số lớn hơn số lớn nhất hiện có, và ghi rõ lý do đổi tên vào trường `ghi-chu-doi-ten` trong frontmatter của file. (Rút ra từ sự cố 2026-09-19: `19-thu-tuc-hanh-chinh-va-xac-minh-bi-thu.md` trùng số với `19-ten-day-du-nhom5-5tinh.md` do một phiên Claude Code khác tạo trước đó; đã đổi thành `41-thu-tuc-hanh-chinh-va-xac-minh-bi-thu.md`. Đây là lần đụng số file thứ 3 được ghi nhận trong dự án — xem thêm 2 lần trước trong `Build-logs-sodobmhc.md`.)
...
| Các file dữ liệu đánh số (`01-...md` → `41-...md`, tăng dần khi có dữ liệu mới — số KHÔNG liên tục vì đã có 3 lần đổi số do đụng độ nhiều phiên, xem luật 22) | Dữ liệu thô đã kiểm chứng, chờ ráp vào `data/*.json` ở Phase 10 | Acc Pro (sau khi ráp từ acc Free) |
| `Danh-gia-tong-ket-sodobmhc.md` | Đánh giá tổng kết dự án theo góc nhìn bên thứ ba (điểm mạnh, khoảng trống, lộ trình hoàn thiện + ước lượng nguồn lực) — viết định kỳ khi có mốc lớn, không phải mỗi phiên | Acc Pro |
| `Co-che-chong-loi-thoi-sodobmhc.md` | Phân tầng tốc độ lỗi thời của dữ liệu, nhiệm kỳ/mốc chính trị cần theo dõi, ngưỡng cảnh báo + lịch kiểm tra định kỳ theo tầng, mốc thay đổi đã biết trước | Acc Pro, cập nhật mục "Lịch các mốc đã biết trước" mỗi khi phát hiện thêm |
| `Script-youtube-sodobmhc.md` | Kịch bản video giới thiệu dự án (không phải file quản lý tiến độ) | Acc Pro, theo yêu cầu user |
...
| `AGENTS-sodobmhc.md` (gốc thư mục dự án) | Quy tắc rút gọn cho AI **code editor** (Claude Code/Cline) — khác đối tượng với file này (file này dành cho AI **tra cứu dữ liệu**) | Claude Code, do Pro soạn brief (2026-09-18) |
...
| `.git` + remote GitHub | Backup lịch sử toàn bộ thư mục này — repo riêng, TÁCH BIỆT khỏi repo Obsidian Vault ở thư mục cha. Remote: `https://github.com/haphanhp/sodo-bo-may-hanh-chinh` (nhánh `main`). Lần backup đầu: commit `ef8f239` (2026-09-18) — gồm 9 schema, `AGENTS-sodobmhc.md`, toàn bộ file `.md` housekeeping + dữ liệu thô | Claude Code (push), Pro/user (theo dõi) |

Tất cả file `.md` trong thư mục dự án đều gắn tag `sodobmhc` (frontmatter YAML) để lọc trong Obsidian. `schemas/*.json` và `AGENTS-sodobmhc.md` không có frontmatter (không phải file Obsidian note, là file cấu hình/kiến trúc app).
...
Xem chi tiết trong `Roadmap-sodobmhc.md`. Tình trạng 2026-09-18: Phase 0 và Phase 1 đã xong, đang mở **Phase 2 — data engine**. Dữ liệu thô trong phạm vi mới xem như đã đủ để bắt đầu ráp `data/*.json` ở Phase 10; việc tra cứu qua acc Free tạm dừng, chỉ mở lại khi cần vá điểm thiếu ở cấp thượng tầng/cấp tỉnh.
